# Mihomo Rule Provider

这是一个用于存放 **Mihomo Rule Provider 规则片段** 的仓库。

规则按不同服务或用途分类整理，主要提供可直接订阅的 TXT 规则文件，方便在 Mihomo 配置中通过 `rule-providers` 引用。

## Raw 订阅地址

请直接使用对应规则文件的 GitHub Raw 地址：

OpenAi 🤖

```text
https://raw.githubusercontent.com/Baidaidai-GFWD-origin/RuleProviders/main/openai.text
```
SiriAi 🥃

```text
https://raw.githubusercontent.com/Baidaidai-GFWD-origin/RuleProviders/main/siri.text
```

## 说明

规则内容会根据实际使用情况持续调整。

如发现缺失、失效或误匹配，欢迎提交 Issue 或 Pull Request。